  ...
  for variation_data in variation_data_entries:
    ...
    for calt in alts:
      if ( ((alt == ".") or ((len(alt) == 1) and (calt == alt)) or ((len(alt) != 1) and (len(calt) == len(alt)))) and 
        ...
        for elem in variation_data[finfo].split(';'):
          if elem.startswith('AF='):
            if AF1000g == None: AF1000g = float(elem[3:])
            else: AF1000g += float(elem[3:])
          elif elem.startswith('ASN_AF='):
            if ASN_AF == None: ASN_AF = float(elem[7:])
            else: ASN_AF += float(elem[7:])
          elif elem.startswith('AMR_AF='):
            if AMR_AF == None: AMR_AF = float(elem[7:])
            else: AMR_AF += float(elem[7:])
          elif elem.startswith('AFR_AF='):
            if AFR_AF == None: AFR_AF = float(elem[7:])
            else: AFR_AF += float(elem[7:])
          elif elem.startswith('EUR_AF='):
            if EUR_AF == None: EUR_AF = float(elem[7:])
            else: EUR_AF += float(elem[7:])
