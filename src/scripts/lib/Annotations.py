...
import numpy as np
import pandas as pd
from AnalysisLib import get_range_from_tabix
                    ...
                    for elem in self.tabix[0].fetch(res['Chrom'],
                        ...
                        if res['FeatureID'] in elem[8]:
                            ...
                            if iPos < start:
                                ...
                                val = min((iPos - start + 1, 1, True if strand else False),
                                          (end - iPos + 1, 1, False if strand else True))
                            else:
        ...
        for iPos in set([res['Start'], res['End']]):
            for cmaxval in (maxvalDIST/10000, maxvalDIST/1000, maxvalDIST/100, maxvalDIST/10, maxvalDIST):
                try:
                    ...
                    pass
                if minDistTSS != None and minDistTSE != None:
                    break
        ...
        if len(self.score) > 0:
            ...
            for motif in self.score:
                ...
                if res['Type'] in ['INS', 'DEL']:
                    ...
                    if res['Ref'] == 'A':
                        val1 += ACGT[0]/len(sites)
                    elif res['Ref'] == 'C':
                        val1 += ACGT[1]/len(sites)
                    elif res['Ref'] == 'G':
                        val1 += ACGT[2]/len(sites)
                    elif res['Ref'] == 'T':
                        val1 += ACGT[3]/len(sites)
                    else:
                        failed = True

                    if res['Alt'] == 'A':
                        val2 += ACGT[0]/len(sites)
                    elif res['Alt'] == 'C':
                        val2 += ACGT[1]/len(sites)
                    elif res['Alt'] == 'G':
                        val2 += ACGT[2]/len(sites)
                    elif res['Alt'] == 'T':
                        val2 += ACGT[3]/len(sites)
                    else:
                        failed = True
...
# SEGWAY SCORING
segway_order = ['D', 'L0', 'L1', 'F0', 'F1', 'R0', 'R1', 'R2', 'R3', 'R4', 'R5', 'C0', 'C1',
                'H3K9me1', 'TF0', 'TF1', 'TF2', 'TSS', 'GS', 'GE0', 'GE1', 'GE2', 'GM0', 'GM1', 'E/GM'][::-1]
...
class EncodeOpenChrom(FeatureAnnotation):
        ...
        if os.path.exists(self.path):
            ...
            for filename in infile:
                ...
                if os.path.exists(fullname):
                    self.tabixlist.append((pysam.TabixFile(fullname, 'r'), None, None, None,
                                          None, None, None, "%s %s" % (self.name, fullname)))
                else:
    ...
    def _retrieve(self, res):
        ...
        # [(20,locCode),(9,lcomb_pvalue),(11,ldnase_pvalue),(13,lfaire_pvalue),(15,lpolII_pvalue),(17,lctcf_pvalue),(19,lcmyc_pvalue),(10,ldnase_signal),(12,lfaire_signal),(14,lpolII_signal),(16,lctcf_signal),(18,lcmyc_signal)]
        maxscores = [[], [], [], [], [], [], [], [], [], [], [], []]
        newlist = []
    ...
    def _get_score(self, res):
        ...
        if self.score[0] != None:
            res['EncOCC'] = self.score[0]  # '%d'%ocCode
        if self.score[1] != None:
            res['EncOCCombPVal'] = self.score[1]  # '%.3f'%comb_pvalue
        if self.score[2] != None:
            res['EncOCDNasePVal'] = self.score[2]  # '%.3f'%dnase_pvalue
        if self.score[3] != None:
            res['EncOCFairePVal'] = self.score[3]  # '%.3f'%faire_pvalue
        if self.score[4] != None:
            res['EncOCpolIIPVal'] = self.score[4]  # '%.3f'%polII_pvalue
        if self.score[5] != None:
            res['EncOCctcfPVal'] = self.score[5]  # '%.3f'%ctcf_pvalue
        if self.score[6] != None:
            res['EncOCmycPVal'] = self.score[6]  # '%.3f'%cmyc_pvalue
        if self.score[7] != None:
            res['EncOCDNaseSig'] = self.score[7]  # '%.2f'%dnase_signal
        if self.score[8] != None:
            res['EncOCFaireSig'] = self.score[8]  # '%.2f'%faire_signal
        if self.score[9] != None:
            res['EncOCpolIISig'] = self.score[9]  # '%.2f'%polII_signal
        if self.score[10] != None:
            res['EncOCctcfSig'] = self.score[10]  # '%.2f'%ctcf_signal
        if self.score[11] != None:
            res['EncOCmycSig'] = self.score[11]  # '%.2f'%cmyc_signal
        return res
...
regElement_order = ['Promoter', 'TF binding site', 'Enhancer',
                    'Promoter Flanking Region', 'CTCF Binding Site', 'Open chromatin']
...
class EsmScoreMissense(Annotation):
    name = "EsmScoreMissense"
    consequence = True

    def process(self, res):
        if self.name in res["Info"]:
            # print(res['Info'][self.name])
            for esmEntry in res["Info"][self.name]:
                fields = esmEntry.split("|")
                if fields[0] == res["Feature"]:
                    res[self.name] = fields[
                        1
                    ]  # Wert fuer esm score aus info einrag in vep vcf schreiben(?)
        return res


class EsmScoreInFrame(Annotation):
    name = "EsmScoreInFrame"
    consequence = True
...
class EsmScoreFrameshift(Annotation):
    name = "EsmScoreFrameshift"
    consequence = True
...
class RegSeq(Annotation):
    name = "RegSeq"
    consequence = False
    features = ["RegSeq%d" % i for i in range(8)]

    def process(self, res):
        for feature in self.features:
            if feature in res["Info"]:
                res[feature] = res["Info"][feature][0]
        return res


class Aparent2(TabixAnnotation):
    name = "Aparent2"
    path = "/Aparent/aparent2_hg38.csv.gz"
    consequence = True

    def _get_score(self, res):
        scores = []
        for hit in self.score:
            if (hit[3] == res["Ref"] and hit[4] == res["Alt"]) or (
                hit[4] == res["Ref"] and hit[3] == res["Alt"]
            ):
                scores.append(hit)

            # take rather the max value if many exist (due to the different hexamers, see the paper)
        if len(scores) > 0:
            scores = pd.DataFrame(scores)
            if res["GeneName"] is scores.iloc[:, 1]:
                score = scores[scores.iloc[:, 1] == res["GeneName"]][5].max()
            else:
                score = scores[5].max()
...
class ZooPriPhyloP(TabixAnnotation, ScoreHighest):
    name = "ZooPriPhyloP"
    zerobased = True
    rangescore = True


class ZooVerPhyloP(TabixAnnotation, ScoreHighest):
    name = "ZooVerPhyloP"
    zerobased = True
    rangescore = True


class ZooRoCC(TabixAnnotation, ScoreHighest):
    name = "ZooRoCC"
    zerobased = True
    rangescore = True


class ZooUCE(TabixAnnotation, ScoreHit):
    name = "ZooUCE"
    zerobased = True
    rangescore = True


class Roulette(TabixAnnotation, ScoreHit):
    name = "Roulette"
    features = ["Roulette-FILTER", "Roulette-MR", "Roulette-AR"]

    def _get_score(self, res):
        for hit in self.score:
            if (hit[2] == res["Ref"] and hit[3] == res["Alt"]) or (
                hit[3] == res["Ref"] and hit[2] == res["Alt"]
            ):
                hit = pd.Series(hit).replace(".", "NA")
                res["Roulette-FILTER"] = hit[4]
                res["Roulette-MR"] = hit[8]
                res["Roulette-AR"] = hit[5]
